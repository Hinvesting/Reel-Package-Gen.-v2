...
export const generateImage = async (prompt: string, aspectRatio: string, tone: string, referenceImages: string[] = []): Promise<string> => {
    try {
        let toneInstruction = '';
        if (tone.toLowerCase() === 'realistic') {
            toneInstruction = 'Ensure the image is photorealistic, with highly detailed skin textures, natural complexions, and lifelike features. ';
        }
        
        const enhancedPrompt = `Generate a cinematic image with a ${aspectRatio} aspect ratio. ${toneInstruction}${prompt}`;
...
export const editImage = async (base64ImageData: string, prompt: string): Promise<string> => {
    try {
        ...
        if (response?.text) {
            throw new Error(`Model returned an unexpected text response: ${response.text}`);
        }

        console.error("Image editing response did not contain an image part:", JSON.stringify(response, null, 2));
        throw new Error("No edited image was generated by the model. The request might have been blocked or the response was empty.");
